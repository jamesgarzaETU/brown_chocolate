...
try:
    ...
    boto3 = None # Handle missing boto3 for local mode
...
class ParquetPipeline:
    ...
    def __init__(self, s3_bucket, customer, s3_client=None, local_data_dir=None):
        """
        ...
        Args:
            ...
            local_data_dir (str, optional): absolute path to local data directory. 
                                            If set, overrides S3 and uses local files.
        """
        ...
        if self.local_data_dir:
            self.s3 = None
            logger.info(f"Initialized ParquetPipeline for {customer} in LOCAL MODE")
            logger.info(f"  Local Dir: {self.local_data_dir}")
            
            # Ensure local directories exist
            os.makedirs(os.path.join(self.local_data_dir, self.raw_tables_prefix), exist_ok=True)
            os.makedirs(os.path.join(self.local_data_dir, self.metadata_prefix), exist_ok=True)
            
        else:
            if boto3 is None:
                raise ImportError("boto3 is required for S3 mode but is not installed.")
    ...
    def get_last_update_date(self, table_name):
        ...
        if self.local_data_dir:
            # Local File Mode
            metadata_path = os.path.join(self.local_data_dir, metadata_key)
            if os.path.exists(metadata_path):
                with open(metadata_path, 'r') as f:
                 ...
                 logger.info(f"No local metadata found for {table_name}")
                 return None
    ...
    def update_last_update_date(self, table_name, date):
        ...
        if self.local_data_dir:
            ...
            try:
                ...
                logger.error(f"Error updating local metadata for {table_name}: {e}")
            return
    ...
    def read_parquet_from_s3(self, table_name):
        ...
        if self.local_data_dir:
            # Local File Mode
            local_path = os.path.join(self.local_data_dir, self.raw_tables_prefix, f'{table_name}.parquet')
            if os.path.exists(local_path):
                 logger.info(f"Reading {table_name} from Local: {local_path}")
                 try:
                    df = pd.read_parquet(local_path, engine='pyarrow')
                    logger.info(f"✓ Loaded {len(df):,} rows from {table_name}")
                    return df
                 except Exception as e:
                    logger.error(f"Error reading local parquet {table_name}: {e}")
                    return None
            else:
    ...
    def write_parquet_to_s3(self, df, table_name, compression='snappy'):
        ...
        if self.local_data_dir:
            ...
            local_path = os.path.join(self.local_data_dir, self.raw_tables_prefix, f'{table_name}.parquet')
            try:
                logger.info(f"Writing {len(df):,} rows to {table_name} locally...")
                df.to_parquet(local_path, engine='pyarrow', compression=compression, index=False)
                logger.info(f"✓ Saved to {local_path}")
                return
            except Exception as e:
                logger.error(f"Error writing local parquet {table_name}: {e}")
                raise
